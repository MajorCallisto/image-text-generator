...
const bufferColorful = (width, height, textOptions = {}) => {
  const { fontSize = 128, wrapBreak = 6, passPhraseLength = 3, text = [], xStart = 32, yStart = 128 } = textOptions;
  let lastPadding = yStart;

  const passPhrase =
    text.length === 0 ? generate({ length: passPhraseLength, separator: '-', uppercase: true, numbers: false }).split('-') : text;
...
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}">
	...
	<radialGradient id="radialGradient1" cx="${randomNumber(width)}" cy="${randomNumber(height)}" r="2000" gradientUnits="userSpaceOnUse">
	...
	<radialGradient id="radialGradient2" cx="${randomNumber(width)}" cy="${randomNumber(height)}" r="2000" gradientUnits="userSpaceOnUse">
		<stop offset="0.1" style="stop-color:${randomHSL(0, 100, 30, 360, 100, 85)}" />
		<stop offset="0.8" style="stop-color:${randomHSL(0, 100, 30, 360, 100, 85)};stop-opacity:0" />
	</radialGradient>
    ...
    ${passPhrase.map((phrase, index) => {
      ...
      if (phrase.length > wrapBreak) {
        svgText = `<text x="${xStart}" y="${lastPadding}" class="text" fill="white">${phrase.substring(0, wrapBreak)}</text>
            <text x="${xStart}" y="${lastPadding + fontSize * 0.85}" class="text" fill="white">${phrase.substring(
          wrapBreak,
          phrase.length,
        )}</text>`;
...
for (let i = 0; i < 5; i++) {
  sharpDecorator(640, 640, { fontSize: 96, wrapBreak: 8 }).toFile(`./sample_${i}.png`);
}
